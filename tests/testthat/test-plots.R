...
test_that("Test against reference results", {
  ...
  expect_snapshot_file(save_svg(plot_1), "basic-example-plot.svg")
  ...
  plot_2 <- plot(proj)
  expect_snapshot_file(save_svg(plot_2), "evd-proj.svg")

  plot_3 <- plot(proj, boxplots = TRUE, outliers = FALSE)
  expect_snapshot_file(save_svg(plot_3), "evd-proj-box-no-outliers.svg")

  plot_4 <- plot(proj, ribbon = FALSE)
  expect_snapshot_file(save_svg(plot_4), "evd-proj-no-ribbon.svg")

  plot_5 <- plot(proj, boxplots = FALSE, linetype = 2, linesize = 3)
  expect_snapshot_file(save_svg(plot_5), "evd-proj-no-box-custom-lines.svg")
  
  plot_6 <- plot(proj, boxplots = TRUE, boxplots_color = "red")
  expect_snapshot_file(save_svg(plot_6), "evd-proj-red-box.svg")

  plot_7 <- plot(proj, quantiles = FALSE, ribbon = FALSE, boxplots = TRUE)
  expect_snapshot_file(save_svg(plot_7), "evd-proj-box-only.svg")

  plot_8 <- plot(proj, quantiles = FALSE)
  expect_snapshot_file(save_svg(plot_8), "evd-proj-ribbon-only.svg")

  plot_9 <- plot(proj, ribbon_color = "red", quantiles = FALSE)
  expect_snapshot_file(save_svg(plot_9), "evd-proj-red-ribbon.svg")
  ...
  expect_snapshot_file(save_svg(plot_10), "evd-proj-full-red-ribbon-narrow-range.svg")
     
  
  ## adding projections to incidence::incidence plot
  plot_11 <- plot(i) %>% add_projections(proj)
  expect_snapshot_file(save_svg(plot_11), "evd-proj-with-incidence-incidence.svg")
  
  plot_12 <- plot(i) %>% add_projections(proj, boxplots = TRUE)
  expect_snapshot_file(save_svg(plot_12), "evd-proj-with-incidence-incidence-no-box.svg")
  
  plot_13 <- 
    plot(i) %>% 
    add_projections(proj, quantiles = FALSE, ribbon = FALSE, boxplots = TRUE)
  expect_snapshot_file(save_svg(plot_13), "evd-proj-with-incidence-incidence-box-only.svg")
  ...
  expect_snapshot_file(save_svg(plot_14), "evd-proj-with-incidence-incidence-and-custom.svg")
